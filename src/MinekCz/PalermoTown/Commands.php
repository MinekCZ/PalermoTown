...
class Commands extends Command
{
    ...
    public function __construct(PalermoTown $palermoTown)
    {
        parent::__construct("palermotown", "PalermoTown", null, ["pt"]);
    ...
    public function Editor(Player $sender, array $args) 
    {
        ...
        switch($args[0]) 
        {
                ...
                if(is_array($this->data[$key])) 
                {
                    ...
                    switch($value_arg) 
                    {
                        ...
                        case "player_pos":
                            ...
                            $value = PalermoTown::VecToString($sender->getPosition()->round(0, PHP_ROUND_HALF_DOWN));

                            break;
                        case "block_look":
                        case "look_block":
                            $block = $sender->getTargetBlock(4, []);
                            $value = PalermoTown::VecToString($block->getPosition());
                            break;
                        case "pos_look":
                        case "look_pos":
                            $value = PalermoTown::VecToString($sender->getTargetBlock(4, [])->getPosition()->addVector(new Vector3(0, 1, 0)));
                            break;
                        default:
            ...
            case "savelevels":
                ...
                $this->palermoTown->saveMap($this->getServer()->getWorldManager()->getWorldByName($game));

                if($this->data["savelobby"] == "true" && $lobby != $game) 
                {
                    $sender->sendMessage("\n§7Saving §a\"{$lobby}\"§7...");
                    $this->palermoTown->saveMap($this->getServer()->getWorldManager()->getWorldByName($lobby));
                } elseif ($lobby != $game)
                {
    ...
    {
        return $this->palermoTown->getDataFolder();
    }
